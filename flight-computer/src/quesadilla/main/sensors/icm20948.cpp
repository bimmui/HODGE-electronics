...
void ICM20948::configureAK09916()
{
    ...
    uint8_t tmp[1] = {0};
    icm20948_read(ak09916_dev_handle, AK09916_WIA2, tmp, sizeof(tmp));
    // printf("the thing: %u\n", tmp[0]);

    // continuous measurmeent mode 4 (100 Hz)
    const uint8_t reg_and_data2[] = {AK09916_CNTL2, 0x08};
    icm20948_write(ak09916_dev_handle, reg_and_data2, sizeof(reg_and_data2));
...
void ICM20948::activateI2CBypass()
{
    ...
    // uint8_t tmp1[1] = {0};
    // icm20948_read(icm20948_dev_handle, ICM20948_USER_CTRL, tmp1, sizeof(tmp1));
    ...
    // uint8_t tmp2[1] = {0};
    // icm20948_read(icm20948_dev_handle, ICM20948_INT_PIN_CFG, tmp2, sizeof(tmp2));
    // printf("init: the thing for bypass: %u\n", tmp2[0]);
    // uint8_t tmp3[1] = {0};
    // icm20948_read(icm20948_dev_handle, ICM20948_USER_CTRL, tmp3, sizeof(tmp3));
    // printf("init: the thing for master: %u\n", tmp3[0]);
}
...
float ICM20948::getTemp()
{
    ...
    uint8_t data_rd[2] = {0};
    icm20948_read(icm20948_dev_handle, 0x39, data_rd, sizeof(data_rd));

    int16_t temp = (int16_t)((data_rd[0] << 8) + (data_rd[1]));

    float final_temp = ((temp - 21) / 333.87) + 21;
    return final_temp;
}
