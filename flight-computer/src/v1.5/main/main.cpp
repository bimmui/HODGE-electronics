...
extern "C" void app_main(void)
{
    ...
    WeightedMovingAverage wma(16);
    float max_alt = 0;
    bool reached_apo = false;
    ...
    while (true)
    {
        ...
        float alt = sample.altitude;
        flightlog.bmp_altitude = alt;
        wma.addSample(alt);
        ...
        {
            if (alt > (wma.getAverage() + (3.00 * STD)))
            {
                max_alt = alt;
            }
            if (alt < (wma.getAverage() + (3.00 * STD)))
        ...
        if (reached_apo and (wma.getAverage() < MAIN_DEPLOY))
        {
